...
def clean_ingredients(dish_name, dish_ingredients):
    ...
    cleaned_ingredients_list = tuple([dish_name, set(dish_ingredients)])
...
def check_drinks(drink_name, drink_ingredients):
    ...
    if set(drink_ingredients).isdisjoint(ALCOHOLS):
        drink_category = drink_name + ' Mocktail'
    else:
        drink_category = drink_name + ' Cocktail'
...
def tag_special_ingredients(dish):
    ...
    special_ingredient_of_dish = set(dish[1]).intersection(SPECIAL_INGREDIENTS)
    return tuple([dish[0], special_ingredient_of_dish])
...
def singleton_ingredients(dishes, intersection):
    ...
    for dish in dishes:
        singleton_ingredients_set ^= dish
    return singleton_ingredients_set - intersection
