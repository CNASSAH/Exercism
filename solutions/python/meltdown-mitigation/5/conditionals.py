...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    balance = temperature < 800 and neutrons_emitted > 500 and temperature * neutrons_emitted < 500000
    return balance
...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    efficiency_ratio = (generated_power/theoretical_max_power) * 100
    if efficiency_ratio >= 80:
        return 'green'
    if efficiency_ratio < 80 and efficiency_ratio >= 60:
        return 'orange'
    if efficiency_ratio < 60 and efficiency_ratio >= 30:
        return 'red'
    return 'black'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    if (temperature * neutrons_produced_per_second) < 0.9 * threshold:
        return 'LOW'
    if (temperature * neutrons_produced_per_second) > threshold * (1 - 0.1)  and (temperature * neutrons_produced_per_second) < threshold * (1 + 0.1):
        return 'NORMAL'
    else:
        return 'DANGER'
