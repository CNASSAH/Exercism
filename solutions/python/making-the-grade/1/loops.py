...
def round_scores(student_scores):
    ...
    while student_scores:
        new_student_score.append(round(student_scores.pop(0)))
...
def letter_grades(highest):
    ...
    while min_next_level < highest:
        new_american_score.append(min_next_level)
        min_next_level += grade_interval

    return new_american_score
...
def student_ranking(student_scores, student_names):
    ...
    for index, name in enumerate(student_names):
        ranking_students.append(f"{index + 1}. {name.title()}: {student_scores[index]}")
    return ranking_students
